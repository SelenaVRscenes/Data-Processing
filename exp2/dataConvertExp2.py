"""
This is a data conversion and visualisation designed especially for 
the Experiment dedicated to motion randomness.
"""
...
for id in iserIdList:
        ...
        if (( id in filename ) and (ext == '.csv') and ("Coord" in filename) and not( "init" in filename )):
            ...
            if os.path.isfile(f):
                ...
                for i, row in tempDF.iterrows():
                    ...
                    if not((X< -400) or (X> 400) or (Y> 400)  or (Y< -400)):
                        Xarr.append(X)
                        Yarr.append(Y)
                        Zarr.append(Z)
            ...
            if (len(Xarr)>0 and len(Yarr)>0):
                ...
                if (len(Xarr_DP1)>0 and len(Yarr_DP1)>0):
                    model = np.polyfit(Xarr_DP1, Yarr_DP1, 0)
                    
                    predict = np.poly1d(model)
                    y_lin_reg1 = predict(Xarr_DP1)            
                    plt.plot(Xarr_DP1, y_lin_reg1, color=[0.2, 0.05*levelColor,1 -  0.05*levelColor], label= lbl)
                ...
                if (len(Xarr_DP2)>0 and len(Yarr_DP2)>0):
                    ...
                    plt.plot(Xarr_DP2, y_lin_reg2, color=[0.2,  0.05*levelColor,1 -  0.05*levelColor], label= lbl)
                ...
                if (xInter != 0 and yInter != 0) :
                    plt.plot(xInter, yInter, color=[0.2, 0.05*levelColor,1 - 0.05*levelColor] ,marker='h', markerfacecolor='lightgreen', markeredgewidth=12,
             markersize=20, markevery=10)
                ...
                #plt.plot(Xarr, Yarr, '.', color=[cval,abs(0.02*levelColor),1 - abs(0.02*levelColor)], linewidth=0.02 , label=filename[-18:-10])
                                #,figure=plt.figure());
                plt.plot(Xarr, Yarr, '.', color=[cval,1 - abs(0.02*levelColor),1- cval], linewidth=0.02 , label=filename[-18:-10]+str(len(Xarr)))
                                #,figure=plt.figure());
                                
                    
                print(len(Xarr), len(Yarr))
                titles.append(filename[-18:-10]+str(len(Xarr)))
    ...
    foutImg = os.path.join(img_directory,id+"_p0_p1.png")   
    #plt.legend(bbox_to_anchor =(1.75,2.15), ncol = 2) 
    
    lgnd  = plt.legend( loc='lower left' , ncol = 2, frameon=True, prop={'size': 12});
    
    #lgnd.legendHandles[0].legmarker.set_markersize(40)# ._legmarker.set_markersize(40)
    ...
    foutStat = os.path.join(stat_directory,"deviation"+"_p0_p1.csv")   
    statDF.to_csv(foutStat)
    
    
foutStat = os.path.join(stat_directory,"deviation"+"_p0_p1.csv")   
statDF.to_csv(foutStat)
foutStat = os.path.join(stat_directory,'deviation_p0_p1.xlsx') 
...
for i, id in enumerate (iserIdList):
    ...
    if i == int (len(iserIdList) / 2): 
        ...
        lgnd  = plt.legend( loc='lower left' , ncol = 4, frameon=True, fontsize = 'x-small');
        
        foutImg = os.path.join(img_directory,"deviation_part1_p0_p1.png")   
        plt.savefig(f'{foutImg}', dpi=my_dpi)
...
foutImg = os.path.join(img_directory,"deviation_part2_p0_p1.png")   
plt.savefig(f'{foutImg}', dpi=my_dpi)
