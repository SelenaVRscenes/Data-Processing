...
def resample(time, xdata, ydata, newFrameRate):
	...
	xInter = interp1d(time, xdata, kind='linear')
	new_xdata = xInter(newTime)

	#resample y data accordingly
	xInter = interp1d(time, ydata, kind='linear')
	new_xdata = xInter(newTime)
	...
	xdataFilter = signal.sosfilt(sos, new_xdata)
	ydataFilter = signal.sosfilt(sos, new_ydata)
...
FPSTarget = 40
my_dpi = 300
nSlopes = 2 # for interpalatopon 

XEs = -110 # ES character
YEs = -121
XNe = -116 # Neurotic character
YNe = 112
...
df_csv_directory= os.path.join( path_of_the_directory, "csvCleanedData")
...
for id in iserIdList:
    ...
    for filename in os.listdir(df_csv_directory): 
        
        name, ext = os.path.splitext(filename)
        
        if (( id in filename ) and (ext == '.csv') and ("Coord" in filename) and not( "init" in filename ) and filename[-11:-10] != '0' ) :
            f = os.path.join(df_csv_directory, filename)
            if os.path.isfile(f):
                levelColor+=0.1
                ...
                trackDF =pd.read_csv(f)  
                print(trackDF["X"])
                
                Xarr = trackDF['X'].to_numpy()
                Yarr = trackDF['Y'].to_numpy()
                YAarr = trackDF['YA'].to_numpy()
                
                for index, row in trackDF.iterrows():
                    angleNeu = math.degrees(  math.asin( (row['Y'] - YNe)/np.sqrt((row['Y'] - YNe)*(row['Y'] - YNe)+(row['X'] - XNe)*(row['X'] - XNe)) )   )
                    angleEs = math.degrees(  math.asin( (row['Y'] - YEs)/np.sqrt((row['Y'] - YEs)*(row['Y'] - YEs)+(row['X'] - XEs)*(row['X'] - XEs)) )  )
                    AngleMotionNeuArr = np.append(AngleMotionNeuArr, angleNeu)    
                    AngleMotionEsArr = np.append(AngleMotionEsArr, angleEs) 

                print(AngleMotionNeuArr)
                plt.plot(AngleMotionNeuArr, '.', color=[0.2, levelColor ,1 - levelColor], linewidth=1.4 , label="NE dev angle "+filename[-18:-10])
                plt.plot(AngleMotionEsArr,'.', color=[0.8, levelColor ,1 - levelColor], linewidth=1.4 , label="ES dev angle "+filename[-18:-10])
                plt.plot(YAarr, '-', color=[0.2, 0.5 ,1 - levelColor], linewidth=2.4 , label="Headset angle "+filename[-18:-10])
                
    lgnd  = plt.legend( loc='lower center' , ncol = 4, frameon=True, fontsize = 'x-small');

    foutImg = os.path.join(img_directory,"Angle_"+str(id)+"_resample.png")   
    plt.savefig(f'{foutImg}', dpi=my_dpi)
