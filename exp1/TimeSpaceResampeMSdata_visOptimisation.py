    ...
    for filename in os.listdir(raw_data_csv_directory): 
        ...
        if (( id in filename ) and (ext == '.csv') and ("Coord" in filename) and not( "init" in filename ) and filename[-11:-10] != '0' ) :
            ...
            if (len(Xarr)>0 and len(Yarr)>0):
                ...
                print(max(np.array(resTSDF["timestamp"])), 
                    max(t))    
...
#time
fout = os.path.join(stat_directory,"_raw_")   
timeComplCalculation(mergedDF, fout)

fout = os.path.join(stat_directory,"_resTS"+str(FPSTarget))   
timeComplCalculation(mergedTSDF, fout)

fout = os.path.join(stat_directory,"_resTSXY"+str(FPSTarget))   
timeComplCalculation(mergedTSXYDF, fout)


# Calculate min distance 
fout = os.path.join(stat_directory,"_raw_")   
distCalculation(mergedDF, fout)

fout = os.path.join(stat_directory,"_resTS"+str(FPSTarget))   
distCalculation(mergedTSDF, fout)

fout = os.path.join(stat_directory,"_resTSXY"+str(FPSTarget))   
distCalculation(mergedTSXYDF, fout)
...
plt.clf()
plt.figure(figsize=(10,10), dpi= sdpi)

repList = list(mergedTSDF.rep.unique()) # unique levels list
countElem = 0  

for lvl in constLevelList:
    
    countElem = 0
    lvlID = constLevelList.index(lvl)
    condTitle = str(lvlID +1)
    for id in iserIdList:
        
        for rep in repList:
            
            cDF1 = mergedTSDF[mergedTSDF['level']==lvl]
            cDF2 = cDF1[cDF1['ID']==id]
            cDF = cDF2[cDF2['rep']==rep]
            
            disp = cDF[ (cDF["X"]>-200) ].sort_values(by=['timestamp'])
                  
            if countElem ==0:
                plt.plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (1.6), label="Condition "+condTitle)
            else:    
                plt.plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (1.6))
...
foutImg = os.path.join(img_directory,"ALL"+"_lin"+str(nSlopes)+"_lowPass_" +str(fc) + ".png")      
    
plt.savefig(f'{foutImg}', dpi=sdpi)
...
foutImg = os.path.join(img_directory,"ALL_non_empty_"+"_lin"+str(nSlopes)+"_lowPass_" +str(fc) + ".png")      
...
fig, axs = plt.subplots(1, 2, figsize=(20, 10), dpi=sdpi)
  
repList = list(mergedTSDF.rep.unique()) # unique levels list
countElem = 0  

for lvl in [  constLevelList[-2],constLevelList[-1], constLevelList[4] ]:
    ...
    for id in iserIdList:
        ...
        for rep in repList:
            ...
            if countElem ==0:
                axs[0].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2), label="Condition "+condTitle)
            else:    
                axs[0].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2))
            
            countElem +=1

                
axs[0].plot( Xshift+XEs, -(Yshift+YEs), color='limegreen',marker='h', markerfacecolor='lightgreen', markeredgewidth=12,
             markersize=18, markevery=12)#, label="ES character")
axs[0].plot( Xshift+XNe, -(Yshift+YNe), color='coral', marker='h', markerfacecolor='mistyrose', markeredgewidth=12,
             markersize=18, markevery=12)#, label="NE character")
                                               
  
lgnd  = axs[0].legend( loc='lower left' , ncol = 1, frameon=True, prop={'size': 16});    
...
for lvl in [ constLevelList[5],constLevelList[6], constLevelList[3] ]:
    ...
    for id in iserIdList:
        ...
        for rep in repList:
            ...
            if countElem ==0:
                axs[1].plot( (Xshift+ np.array(disp['X'])), (Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2), label="Condition "+condTitle)
            else:    
                axs[1].plot( (Xshift+ np.array(disp['X'])), (Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2))
            
            countElem +=1


                
axs[1].plot( Xshift+XEs, (Yshift+YEs), color='limegreen',marker='h', markerfacecolor='lightgreen', markeredgewidth=12,
             markersize=18, markevery=12)#, label="ES character")
axs[1].plot( Xshift+XNe, (Yshift+YNe), color='coral', marker='h', markerfacecolor='mistyrose', markeredgewidth=12,
             markersize=18, markevery=12)#, label="NE character")
                                               

  
lgnd  = axs[1].legend( loc='lower left' , ncol = 1, frameon=True, prop={'size': 16});    
#lines = plt.gca().get_lines()
#include = [0,len(constLevelList)]
#legend1 = plt.legend([lines[i] for i in include],[lines[i].get_label() for i in include], loc=2, frameon=True, prop={'size': 16});   
#legend2 = plt.legend([lines[i] for i in [-2,-1]],['ES character','NE character'],loc=3, frameon=True, prop={'size': 20});   
...
#axs[1].title('Trajectories for conditions 4, 6, 7')

plt.title('Trajectories for conditions 4-9')
...
foutImg = os.path.join(img_directory,"ALL_NE_sep_"+"_lin"+str(nSlopes)+"_lowPass_" +str(fc) + ".png")      
...
fig, axs = plt.subplots(1, 3, figsize=(30, 10), dpi=sdpi)
...
for lvl in [  constLevelList[-2],constLevelList[-1] ]:
...
for lvl in [ constLevelList[5],constLevelList[6] ]:
...
for lvl in [  constLevelList[4] ]:
    ...
    for id in iserIdList:
        ...
        for rep in repList:
            ...
            if countElem ==0:
                axs[2].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2), label="Condition "+condTitle)
            else:    
                axs[2].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2))
...
axs[2].plot( Xshift+XEs, -(Yshift+YEs), color='limegreen',marker='h', markerfacecolor='lightgreen', markeredgewidth=12,
             markersize=18, markevery=12)#, label="ES character")
axs[2].plot( Xshift+XNe, -(Yshift+YNe), color='coral', marker='h', markerfacecolor='mistyrose', markeredgewidth=12,
             markersize=18, markevery=12)#, label="NE character")
                                               

  
lgnd  = axs[2].legend( loc='lower left' , ncol = 1, frameon=True, prop={'size': 16});    

     

foutImg = os.path.join(img_directory,"ALL_NE_3col_"+"_lin"+str(nSlopes)+"_lowPass_" +str(fc) + ".png")      
...
#not emply levels - sides sep with turn point  
...
fig.suptitle('Individual Trajectories and Turning Points for conditions 4-9').set_size(20)
#plt.xlabel('X-position (cm)').set_size(20)
#plt.ylabel('Y-position (cm)').set_size(20)
...
axs[1].set_xlabel('X-position (cm)').set_size(20)
#axs[1].set_ylabel('Y-position (cm)').set_size(20)


#axs[2].set_xlabel('X-position (cm)').set_size(20)
#axs[2].set_ylabel('Y-position (cm)').set_size(20)
#axs[0].set_xlabel('X-position (cm)').set_size(20)
axs[0].set_ylabel('Y-position (cm)').set_size(20)
...
for lvl in [  constLevelList[-2],constLevelList[-1] ]:
    ...
    for id in iserIdList:
        ...
        for rep in repList:
            ...
            if countElem ==0:
                axs[2].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2), alpha = 0.4, label="Path for cond. "+condTitle)
            else:    
                axs[2].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], linewidth= (2.2), alpha = 0.4)
            
            
            cDF1 = resTSstatDF[resTSstatDF['level']==lvl]
            cDF2 = cDF1[cDF1['ID']==id]
            disp = cDF2[cDF2['rep']==rep]
            axs[2].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color='dimgrey', marker = '.', markersize= (34.8))
            
            if countElem ==0:
                axs[2].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (30.2), label="Turn. Point for cond. "+condTitle)
            else:
                axs[2].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (30.2))
...
for lvl in [ constLevelList[5],constLevelList[6] ]:
    ...
    for id in iserIdList:
        ...
        for rep in repList:
            ...
            if countElem ==0:
                axs[1].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], alpha = 0.4,linewidth= (2.2),label="Path for cond. "+condTitle)
            else:    
                axs[1].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], alpha = 0.4,linewidth= (2.2) )
            
            cDF1 = resTSstatDF[resTSstatDF['level']==lvl]
            cDF2 = cDF1[cDF1['ID']==id]
            disp = cDF2[cDF2['rep']==rep]
            axs[1].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color='dimgrey', marker = '.', markersize= (34.8))
             
            if countElem ==0:
                axs[1].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (30.2), label="Turn. Point for cond. "+condTitle)
            else:
                axs[1].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (30.2))
...
for lvl in [  constLevelList[3] ]:
    ...
    for id in iserIdList:
        ...
        for rep in repList:
            ...
            if countElem ==0:
                axs[0].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID],alpha = 0.4, linewidth= (2.2),label="Path for cond. "+condTitle)
            else:    
                axs[0].plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID], alpha = 0.4,linewidth= (2.2)  )
            
            
            cDF1 = resTSstatDF[resTSstatDF['level']==lvl]
            cDF2 = cDF1[cDF1['ID']==id]
            disp = cDF2[cDF2['rep']==rep]
            axs[0].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color='dimgrey', marker = '.', markersize= (34.8))
            
            if countElem ==0:
                axs[0].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (30.2), label="Turn. Point for cond. "+condTitle)
            else:
                axs[0].plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (30.2))
...
foutImg = os.path.join(img_directory,"ALL_NE_3col_turnPoint_"+"_lin"+str(nSlopes)+"_lowPass_" +str(fc) + ".png")      
...
plt.clf()
plt.figure(figsize=(12,10), dpi= sdpi)
...
for id in iserIdList:
    ...
    for rep in repList:
        ...
        for lvl in constLevelList:
            ...
            if countElem ==0:
                plt.plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID],alpha = 0.8, linewidth= (6.6), label="Cond. "+condTitle)
            else:    
                plt.plot( (Xshift+ np.array(disp['X'])), -(Yshift+ np.array(disp['Y'])) , color=colorLevelListRed[lvlID],alpha = 0.8, linewidth= (6.6))
            ...
            cDF2 = cDF1[cDF1['ID']==id]
            disp = cDF2[cDF2['rep']==rep]
            plt.plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color='dimgrey', marker = '.', markersize= (44.8))
             
            if countElem ==0:
                plt.plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (40.2), label="Turn. Point for cond. "+condTitle)
            else:
                plt.plot( (Xshift+ np.array(disp['x'])), -(Yshift+ np.array(disp['y'])) , color=colorLevelListRed[lvlID], marker = '.', markersize= (40.2))
        ...
        plt.title('User trajectories and turning points for different conditions').set_size(20)
        plt.xlabel('X-position (cm)').set_size(20)
        plt.ylabel('Y-position (cm)').set_size(20)
        ...
        plt.legend( loc='lower left' , ncol = 2, frameon=True, prop={'size': 16});    

        foutImg = os.path.join(img_directory,"ALL_00_"+"_lin"+str(nSlopes)+"_lowPass_" +str(fc)+str(id)+"_rep"+str(rep) + ".png")      
